...
func (s *RootStore) initDiskEngine() {
	...
	s.disk = shard.NewDiskEngine(s.root, dir, s.cfg.Workers, s.cfg.RebuildInterval, shard.DiskEngineConfig{
		ShardSize: shard.ShardSizeConfig{
			TargetBytes: int64(s.cfg.ShardTargetMB) << 20,
			MinBytes:    int64(s.cfg.ShardMinMB) << 20,
			MaxBytes:    int64(s.cfg.ShardMaxMB) << 20,
		},
		RebuildThreshold: s.cfg.RebuildThreshold,
		SearchWorkers:    s.cfg.SearchWorkers,
		InlineKB:         s.cfg.ShardInlineKB,
	})
	s.state.Store(StateIndexing)
}
...
func (s *RootStore) reconcile() {
	files := s.listFiles()
	s.total.Store(int64(len(files)))
	// auto engine: check if source size warrants disk engine
	if s.cfg.Engine == "auto" && s.engineType == "mem" {
		var totalMB int64
		for _, f := range files {
			totalMB += f.size
		}
