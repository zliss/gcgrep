...
func cmdDaemon() int {
	// Disk engine: after build, live heap is ~30MB but Go holds virtual
	// pages. GOMEMLIMIT makes the GC return memory more aggressively.
...
func streamSearch(conn net.Conn, o cliOpts) int {
	...
	defer out.Flush()
	dec := json.NewDecoder(bufio.NewReaderSize(conn, 256*1024))
	matched := false
	progressShown := false
	var lr lineReader
